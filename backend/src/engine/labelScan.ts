...
const PROMPT = `This is a photo of a food package's Nutrition Facts label. Read it carefully and extract the fields in the schema, normalizing every nutrient amount to a per-100-gram basis — labels state amounts per serving, so use the serving size in grams printed on the label to convert (e.g. a 55g serving with 220 calories is 400 calories per 100g). Also report the serving size in grams and the label's own serving description (e.g. "2/3 cup") separately, unconverted. If the product name or brand is visible anywhere in the photo, include it. Sodium is in milligrams; every other nutrient is in grams. If a field isn't present on the label or you can't determine it confidently, return null for that field — never guess or estimate a value.`;

export async function scanNutritionLabel(imageBuffer: Buffer, mediaType: "image/jpeg"): Promise<LabelScanResult> {
