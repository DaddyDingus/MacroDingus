...
- **Cookware tare weights** are per-user calculator inputs, not recipe properties. Recipe creation may subtract a saved empty pot/dish from the combined scale weight, but persists only the resulting net `totalWeightGrams`; editing or deleting cookware must never alter saved recipes.
...
Dashboard alone marks that surface with `data-pull-to-refresh`; crossing the hook's resisted top-pull threshold dispatches its refresh event and uses `forceRefreshApp()` so the gesture retains the old button's wait-for-health-before-clearing-caches safety. Other screens keep bounce-only overscroll.
