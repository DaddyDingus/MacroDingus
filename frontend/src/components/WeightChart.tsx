...
function CustomTooltip({ active, payload, label }: any) {
  ...
  const trend = payload.find((p: any) => p.dataKey === "trendKg");
  const scale = payload.find((p: any) => p.dataKey === "weightKg");
  return (
    <div className="bg-surface-raised border border-line rounded-md px-3 py-2 text-xs">
      ...
      {trend && (
        <p className="flex items-center gap-2">
          ...
          <span className="tabular text-ink">{trend.value.toFixed(1)} kg</span>
      ...
      {scale && (
        <p className="flex items-center gap-2 mt-0.5">
          ...
          <span className="tabular text-ink">{scale.value.toFixed(1)} kg</span>
