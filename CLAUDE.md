...
Routing/bundling: `App.tsx` only keeps the Dashboard (`/`) and Food log (`/log`) screens in the eager bundle — everything else (`Strategy`, `More`, `Photos`, and every detail page: weight/macros/expenditure/energy-balance/goal-progress/habits-weigh-ins/habits-logging/`nutrition/:metric`) is `React.lazy`-loaded per route. Within the Dashboard itself, the bento-tile rendering is further split into its own lazy chunk (`components/DashboardTileSections.tsx`, renamed from the original `DashboardInsights.tsx`) because it's the only part of that screen touching `recharts` (~420kB); `components/DashboardEditorSheet.tsx` (the "Customise dashboard" editor) is also lazy-split even though it touches no chart code, purely because it's infrequently opened — don't import chart components (or other rarely-used, sizeable UI) into an eagerly-loaded screen without lazy-splitting them the same way; check the built bundle sizes (`npm run build` output) if you add one somewhere new. The barcode scanner (`@zxing/*`, ~400kB) is lazy-loaded the same way, and for the same reason it doesn't use the native `BarcodeDetector` API — that's unsupported in Firefox.

The (+) nav button and the Dashboard's 4 pinned shortcut buttons both drive the same underlying flow (`components/QuickActionFlow.tsx`) rather than duplicating the meal-picker/quick-add/log-weight/etc. state machine in two places — extend that one component if you add a new quick action rather than special-casing either entry point. The shortcut buttons themselves float in `components/ShortcutsBar.tsx`, a `fixed` bar just above `BottomNav`, Dashboard-only (not global chrome — the FAB already covers every other screen), hidden on scroll-down and shown on scroll-up via `lib/useHideOnScroll.ts` (rAF-throttled, small dead-zone threshold, always visible near the top).

**Dashboard tile system** (`lib/dashboardLayout.tsx`, `components/DashboardTileSections.tsx`, `components/DashboardEditorSheet.tsx`): the Dashboard is four fixed, non-reorderable categories (Insights & Analytics, Habits, Nutrition, Body Metrics), each holding bento tiles the user can individually toggle on/off and reorder *within that category* via the editor (opened from a full-width "Customise dashboard" tile at the bottom). Persistence is one `localStorage` array of enabled tile ids per category — same Context+localStorage shape as `lib/shortcuts.tsx`/`lib/weightUnit.tsx`, no backend table. `load()` deliberately does **not** try to auto-append catalog tiles missing from a user's stored layout (an earlier version did, to handle "a tile added in a future release," but that's indistinguishable from "the user turned this off," so it silently reverted every toggle-off on reload) — a tile added to the catalog later just won't appear until the editor is opened, same trade-off `shortcuts.tsx` already accepts. `lib/checkins.ts`'s `activeCheckinForDate()` (latest checkin with `date <= target`, computed client-side against the full unbounded `GET /api/checkins` history) answers "which targets were active on some past date," needed by both the Energy Balance chart and every nutrient detail page's daily history list.
