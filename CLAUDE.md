...
**Iron Dingus weight sync** (`routes/weights.ts`: `pushWeightSyncToIronDingus`): every POST/DELETE on `/api/weights` best-effort pushes the full weight history to `http://iron-dingus-backup:3000/api/weight-sync` (container-to-container on the shared `proxy` network, not Tailscale) — never blocks the request, swallows failures. Only **Tristan**'s weigh-ins sync, matched by `users.name` not a hardcoded id — syncing a second person needs a real per-target mapping.
...
**AFCD-seeded local food library**: `foods` has ~1,588 generic staples pre-loaded (`source: 'afcd'`) from the Australian Food Composition Database — real government data, not AI-estimated (idempotent import, `backend/src/scripts/import-afcd-foods.ts`). Local `foods` rows always return before OFF is even queried (`routes/foods.ts`'s `local.length < take` gate), fixing "generic term returns garbage OFF results" without touching OFF/rerank logic. Also added `aminoAcidsJson` (sparse — only ~12% of foods have a real multi-acid panel) and `carbDetailJson`. **Gotcha**: a pre-existing lower-detail food can collide by exact name with an AFCD row and block the better version from ever inserting (happened with "Honey") — fix by hiding the old row (`hiddenAt`) and hand-inserting the AFCD version, not deleting outright.

**Local search relevance** (`routes/foods.ts`'s `GET /api/foods`): had none until 2026-08 — the only sort key was "logged before," so a fresh exact match could rank behind longer names merely containing the word. Fixed with a `relevance()` tier (exact > starts-with > whole-word > substring) as the primary sort key; "logged before" only breaks ties within a tier.

**OFF search reliability** (`engine/openfoodfacts.ts`): neither OFF endpoint is trustworthy alone — legacy `/cgi/search.pl` has no relevance sort, and the current backend derails on descriptive queries (tag-matches on stray words). `searchOffProducts()` queries both in parallel, merges by barcode, and re-ranks by word-overlap against `product_name`. Each source is always asked for ≥8 candidates regardless of the caller's limit — too few remote slots after local matches was too narrow a pool for reranking to work. Every call gets 3 retries + 6s timeout (it 503s often). Rerank still ignores `brands`, no typo tolerance.
...
**AI food entry** (`engine/labelScan.ts` + `engine/describeMeal.ts`, sharing a lazy, optional `getAnthropicClient()` singleton — unset key, both routes just 503): label scanning uses Claude Haiku 4.5 (pure transcription, does its own per-serving→per-100g conversion) with a hand-written JSON Schema, not the SDK's `zodOutputFormat` helper (needs zod's `/v4` subpath, a different namespace from the `zod` v3 request validation already uses everywhere). Meal description uses **Sonnet 5** — needs real judgment — and every returned item is a real, persisted `foods` row: matched by id against the household's own recent foods/logs when confident, else a fresh `source: 'ai_estimate'` row, with the estimate always filled in as a fallback in case the matched id turns out hallucinated.

**Progress photos**: `photos.pose` is nullable (`'front' | 'side' | 'back' | null`) — null means an older/uncategorized shot, still shown in history but excluded from pose comparison. No separate "latest photo by pose" endpoint: derived client-side from the already-fetched full `usePhotos()` list.
...
**Lazy loading**: Dashboard (`/`) and Food log (`/log`) are eager. Everything else is `React.lazy`. `DashboardTileSections.tsx` (recharts) and the barcode scanner (`@zxing/*`, uses `@zxing/browser` not `BarcodeDetector` — Firefox doesn't support it) are lazy-split. Don't import sizeable UI into an eager screen without lazy-splitting.
